...
// returns a HashMap of k-mers and their counts
pub fn read_and_count() -> HashMap<Vec<u8>, HashMap<Vec<u8>, u16>> {
    let params = set_parameters(); // TODO: Remove it as an argument
    let filename = params["file"];
    let mut reader = parse_fastx_file(&filename).expect("valid path/file");
    let mut kmer_count: HashMap<Vec<u8>, HashMap<Vec<u8>, u16>> = HashMap::new();

    // Save k-mer for each sequence in the HashMap and parallelize it with rayon
...
pub fn read_and_count_parallel() -> HashMap<Vec<u8>, HashMap<Vec<u8>, u16>> {
    let params = set_parameters(); // TODO: Remove it as an argument
    let filename = params["file"];
    let mut reader = parse_fastx_file(&filename).expect("valid path/file");
    let mut kmer_count: HashMap<Vec<u8>, HashMap<Vec<u8>, u16>> = HashMap::new();
